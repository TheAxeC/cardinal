...
// storage location that can hold any UDog value. The stack, global variables,
...
// UDog implementation calls these "Obj", or objects, though to a user, all
// values are objects.
//
...
#ifdef UDOG_BITPLATFORM_64
	typedef double udog_number;
	typedef int64_t udog_integer;
	typedef uint64_t udog_uinteger;
#else
	typedef float udog_number;
	typedef int32_t udog_integer;
	typedef uint32_t udog_uinteger;
#endif

#define EPSILON 0.000000001

typedef uint8_t udogByte;
typedef uint16_t udogShort;
typedef uint32_t udogInt;
typedef uint64_t udogLong;

typedef int8_t udogsByte;
typedef int16_t udogsShort;
typedef int32_t udogsInt;
typedef int64_t udogsLong;
...
typedef struct rawObj {
	...
	/// Supported types by UDog:
	///		- Real numbers (floating point)
	///		- Integer numbers
	///		- Garbage collected objects
	/// UDog supports first class objects and first class functions
...
// Primitive result
typedef PrimitiveResult (*Primitive)(UDogVM* vm, ObjFiber* fiber, Value* args, int* numArgs);
...
#define OBJ_VAL(obj) (udogObjectToValue((Obj*)(obj)))

// Returns true if [value] is a bool.
#define IS_BOOL(value) (udogIsBool(value))

// Returns true if [value] is a class.
#define IS_CLASS(value) (udogIsObjType(value, OBJ_CLASS))

// Returns true if [value] is a closure.
#define IS_CLOSURE(value) (udogIsObjType(value, OBJ_CLOSURE))

// Returns true if [value] is a function object.
#define IS_FN(value) (udogIsObjType(value, OBJ_FN))

// Returns true if [value] is an instance.
#define IS_INSTANCE(value) (udogIsObjType(value, OBJ_INSTANCE))

// Returns true if [value] is a string object.
#define IS_STRING(value) (udogIsObjType(value, OBJ_STRING))

// Returns true if [value] is a range object.
#define IS_RANGE(value) (udogIsObjType(value, OBJ_RANGE))

// Returns true if [value] is a fiber object.
#define IS_FIBER(value) (udogIsObjType(value, OBJ_FIBER))

// Returns true if [value] is a table object.
#define IS_TABLE(value) (udogIsObjType(value, OBJ_TABLE))

// Returns true if [value] is a method object.
#define IS_METHOD(value) (udogIsObjType(value, OBJ_METHOD))

// Returns true if [value] is a list object.
#define IS_LIST(value) (udogIsObjType(value, OBJ_LIST))

// Returns true if [value] is a upvalue object.
#define IS_UPVALUE(value) (udogIsObjType(value, OBJ_UPVALUE))

// double -> Value.
#define NUM_VAL(num) (udogNumToValue(num))

// Value -> double.
#define AS_NUM(value) (udogValueToNum(value))
...
void udogTableAdd(UDogVM* vm, ObjTable* list, Value value, Value key);
...
Value udogTableRemove(UDogVM* vm, ObjTable* list, Value index);

// Print an table to the console
void udogTablePrint(UDogVM* vm, ObjTable* list);

HashValue* udogGetTableIndex(ObjTable* table, int ind);
...
static inline bool udogValuesSame(Value a, Value b) {
#if UDOG_NAN_TAGGING
...
// [IS_BOOL].
static inline bool udogIsBool(Value value) {
#if UDOG_NAN_TAGGING
...
static inline bool udogIsObjType(Value value, ObjType type) {
  return IS_OBJ(value) && AS_OBJ(value)->type == type;
}

// Converts the raw object pointer [obj] to a [Value].
static inline Value udogObjectToValue(Obj* obj) {
#if UDOG_NAN_TAGGING
...
static inline double udogValueToNum(Value value) {
#if UDOG_NAN_TAGGING
	DoubleBits data;
	data.bits64 = value;
	return data.num;
...
static inline Value udogNumToValue(double n) {
#if UDOG_NAN_TAGGING
	DoubleBits data;
	data.num = n;
	return data.bits64;
